...
def combine_text(row):
    """🔥 SADECE İNGİLİZCE metinleri birleştirir (title ve abstract sütunları kullanılarak)."""
    
    # Kullanıcının Excel'indeki görsele göre 'title' ve 'abstract' sütunları çekilir.
    title_en = str(row.get("title", "")).strip() 
    descp_en = str(row.get("abstract", "")).strip()
    
    # SADECE İngilizce kısımlar birleştirilir.
...
class SDGDataset(Dataset):
    """Multi-label classification için Dataset."""
...
class XLMRobertaMultiLabel(nn.Module):
    """XLM-RoBERTa + Classification Head mimarisi."""
...
def evaluate(model, dataloader, device, threshold=0.5):
    """Validation/Test değerlendirme döngüsü."""
    model.eval()
    all_preds = []
    all_labels = []
    total_loss = 0
...
def predict_dataframe(df: pd.DataFrame, model, tokenizer, device, batch_size=16, threshold=0.5):
    """Tüm DataFrame için tahmin yapar ve sonuçları formatlar."""
    model.eval()
...
# ====================================================================
# 7. MAIN FONKSİYONU (YENİ AKIŞA GÖRE DÜZENLENDİ)
# ====================================================================

def run_sdg_classifier_new(
    mode: str, 
    train_input_file: str, 
    val_input_file: str, 
    predict_input_file: str,
    output_file: str, 
    model_path: str, 
    epochs: int, 
    batch_size: int, 
    ...
    if mode == "train":
        ...
        # 1. Dengelenmiş Eğitim Verisini Oku
        df_train = read_table(train_input_file, sheet=0)
        
        # 2. Orijinal Doğrulama Verisini Oku
        df_val = read_table(val_input_file, sheet=0)

        # Veri Hazırlığı
        X_train = [combine_text(row) for _, row in df_train.iterrows()]
        y_train = df_train[SDG_LABELS].values.astype(np.int32) 
        
        X_val = [combine_text(row) for _, row in df_val.iterrows()]
        y_val = df_val[SDG_LABELS].values.astype(np.int32)
        
        print(f"Train/Val Veri Setleri Yüklendi. Train: {len(X_train)} (Dengelenmiş), Val: {len(X_val)} (Orijinal)")
        ...
        df = read_table(predict_input_file, sheet=0)
...
if __name__ == "__main__":
    ...
    # 🚨 DİKKAT: Bu dosyalar 'oversampling_araci_yeni.py' tarafından oluşturulmalıdır!
    EGITIM_DOSYASI = "/kaggle/input/sdg-dataset/3egitim_dengelenmis_orneklem.xlsx"
    DOGRULAMA_DOSYASI = "/kaggle/input/sdg-dataset/3dogrulama_orneklem.xlsx"
    
    TAHMIN_GIRDI_DOSYASI = "TF_SDG.xlsx" 
    
    # 🔥 İNGİLİZCE MODELİ KAYIT ADI
    CIKTI_DOSYASI = "tahmin_sonuclari_ingilizce.xlsx"
    MODEL_YOLU = "best_model_ingilizce.pt" 
    ...
    print(f"Seçilen Mod: {CALISMA_MODU.upper()} (SADECE İNGİLİZCE)")
    
    try:
        run_sdg_classifier_new(
            mode=CALISMA_MODU,
            train_input_file=EGITIM_DOSYASI, 
            val_input_file=DOGRULAMA_DOSYASI, 
            predict_input_file=TAHMIN_GIRDI_DOSYASI, 
